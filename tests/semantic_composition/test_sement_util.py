...
class TestSEMENTUtilFunctions:
    ...
    def test_is_sement_isomorphic(sement1, sement2, result):
        assert sement_util.is_sement_isomorphic(sement1, sement2) == result
    ...
    def test_create_variable_roles_dict(sement, gold_roles_dict):
        sement_roles_dict = sement_util.create_variable_roles_dict(sement)
    ...
    def test_create_variable_roles_dict_error(sement):
        with pytest.raises(ValueError):
            sement_util.create_variable_roles_dict(sement)
    ...
    def test_create_hcons_list(sement, gold_hcons_list):
        sement_roles_dict = sement_util.create_hcons_list(sement)
        assert sement_roles_dict == gold_hcons_list

    @staticmethod
    @parametrize_with_cases("sement", cases=CreateHConsList, has_tag="failure")
    def test_create_hcons_list_error(sement):
        with pytest.raises(ValueError):
            sement_util.create_hcons_list(sement)
    ...
    def test_find_slot_overlaps_error(gold_sement, actual_sement):
        with pytest.raises(ValueError):
            sement_util.find_slot_overlaps(gold_sement, actual_sement)

    @staticmethod
    @parametrize_with_cases("gold_sement, actual_sement, gold_overlap_eqs, gold_gold_eqs, gold_actual_eqs",
                            cases=FindVarEqOverlaps, has_tag="success")
    def test_find_eqs_overlaps(gold_sement, actual_sement, gold_overlap_eqs, gold_gold_eqs, gold_actual_eqs):
        test_overlap_eqs, test_gold_eqs, test_actual_eqs = sement_util.find_var_eq_overlaps(gold_sement, actual_sement)
    ...
    def test_find_eqs_overlaps_error(gold_sement, actual_sement):
        with pytest.raises(ValueError):
            sement_util.find_var_eq_overlaps(gold_sement, actual_sement)
    ...
    def test_find_hcons_overlaps(gold_sement, actual_sement, gold_overlap_hcons, gold_gold_hcons, gold_actual_hcons):
        test_overlap_hcons, test_gold_hcons, test_actual_hcons = sement_util.find_hcons_overlaps(gold_sement, actual_sement)
    ...
    def test_find_hcons_overlaps_error(gold_sement, actual_sement):
        with pytest.raises(ValueError):
            sement_util.find_hcons_overlaps(gold_sement, actual_sement)
    ...
    def test_build_isomorphism_report(sement1, sement2, gold_report):
        test_report = sement_util.build_isomorphism_report(sement1, sement2)
