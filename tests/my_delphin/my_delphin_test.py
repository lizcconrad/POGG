...
import pogg.my_delphin.my_delphin as my_delphin

class TestSEMENT(unittest.TestCase):
    """
    Tests the SEMENT class.
    """

    def setUp(self):
        self.sement = my_delphin.SEMENT()

    def test_SEMENT_object(self):
        # assert whether sement object is of the correct type
        self.assertIsInstance(self.sement, my_delphin.SEMENT,
                              "object not of type SEMENT")
...
class TestCodecs(unittest.TestCase):
    ...
    def test__encode_eqs(self):
        gold_encoding = "EQS: < (x11,x7), (x7,i2) >"

        mock_eqs = my_delphin._encode_eqs(self.sement.eqs)
    ...
    def test__encode_eqs_no_eqs(self):
        self.assertEqual(None, my_delphin._encode_eqs(None))

    def test__encode_slots(self):
        gold_encoding = "SLOTS: < ARG2: u3, ARG3: i4 >"

        mock_slots = my_delphin._encode_slots(self.sement.slots)
    ...
    def test__encode_eqs_no_slots(self):
        self.assertEqual(None, my_delphin._encode_slots(None))

    def test__encode_sement(self):
        mock_full_encoding = my_delphin._encode_sement(self.sement, properties=True, lnk=True, indent=True)
        self.assertEqual(self.gold_full_encoding.split(), mock_full_encoding.split())

    def test__encode(self):
        mock_full_encoding = my_delphin._encode([self.sement], properties=True, lnk=True, indent=True)
        self.assertEqual(self.gold_full_encoding.split(), mock_full_encoding.split())

    def test_encode(self):
        mock_full_encoding = my_delphin.encode(self.sement, properties=True, lnk=True, indent=True)
        self.assertEqual(self.gold_full_encoding.split(), mock_full_encoding.split())

    def test_encode_false_options(self):
        mock_full_encoding = my_delphin.encode(self.sement, properties=False, lnk=False, indent=False)
